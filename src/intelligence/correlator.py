...
class EventCorrelator:
    ...
    def correlate(events):
        ...
        for e in events:
            attack_type = normalize_attack(
                EventCorrelator._get_attr(e, "attack_type", "UNKNOWN")
            )
        ...
        logger.debug(f"🔍 Normalized Attacks: {attacks}")
...
class MitreMapper:
    ...
    def map_events(cls, events):
        ...
        for event in events:
            ...
            attack = EventCorrelator._get_attr(
                event,
                "attack_type",
                "UNKNOWN"
            )
        ...
        for event in events:
            ...
            timeline.append({
                ...
                "event_id": (
                    ...
                    if is_dict
                    else getattr(event, "event_id", "N/A")
                ),

                "time": ts,

                "source": (
                    event.get("source", "UNKNOWN_SRC")
                    if is_dict
                    else getattr(event, "source", "UNKNOWN_SRC")
                ),

                "attack": normalize_attack(
                    event.get("attack_type")
                    if is_dict
                    else getattr(event, "attack_type", "UNKNOWN")
                ),

                "severity": (
                    event.get("severity", "LOW")
                    if is_dict
                    else getattr(event, "severity", "LOW")
                ),

                "score": (
                    event.get("score", 0)
                    if is_dict
                    else getattr(event, "score", 0)
                )
            })
