...
class NetworkTools(BaseTool):
    ...
    def network_trend(self):
        return self.execute(
            SplunkQueries.network_trend()
        )

    def failed_connections(self, limit=20):
        return self.execute(
            SplunkQueries.failed_connections(limit)
        )

    def top_source_ips(self, limit=20):
        return self.execute(
            SplunkQueries.top_source_ips(limit)
        )

    def top_destination_ips(self, limit=20):
        return self.execute(
            SplunkQueries.top_destination_ips(limit)
        )

    def port_scan_detection(self):
        return self.execute(
            SplunkQueries.port_scan_detection()
        )